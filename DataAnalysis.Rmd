...
```{r}
birdData <- read_csv("manchesterbirding.csv")
head(birdData)
...
encrichedData <- cbind(allData, spp.rich, shannon)
head(encrichedData)

encrichedData[encrichedData$`Ring-necked Parakeet` == 0,]$has_parakeet <- "No"
encrichedData[encrichedData$`Ring-necked Parakeet` > 0,]$has_parakeet <- "Yes"

head(encrichedData)
```

```{r}
ggplot(encrichedData, aes(x=has_parakeet, y=shannon)) + 
  geom_boxplot()
...
```{r}
nmds <- metaMDS(encrichedData[2:10], k=2, trymax = 30) 

plot(nmds, display = "sites")
with(encrichedData, ordiellipse(nmds, has_parakeet, col=4, lwd=2, draw = "polygon", kind = c("sd"))) 
with(encrichedData, ordispider(nmds, has_parakeet, label=TRUE))
```

```{r}
ef <- envfit(nmds, encrichedData[12:15], na.rm = TRUE, permu = 999) 
ef
```
...
```{r}
plot(nmds, display = "sites")
with(encrichedData, ordiellipse(nmds, has_parakeet, col=4, lwd=2, draw = "polygon", kind = c("sd")), label=T) 
plot(ef, p.max = 0.2)
with(encrichedData, ordispider(nmds, has_parakeet, label=TRUE))
```
